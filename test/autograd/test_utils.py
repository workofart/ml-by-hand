...
class TestUtils(TestCase):
    ...
    def test_train_test_split(self):
        X_train, X_test, y_train, y_test = utils.train_test_split(self.X, self.y, test_size=0.2)
        ...
        X_train, X_test, y_train, y_test = utils.train_test_split(self.X_empty, self.y_empty, test_size=0.2)
    ...
    def test_accuracy(self):
        self.assertEqual(utils.accuracy(self.y, self.y), 1.0)
        self.assertEqual(utils.accuracy(self.y, 1-self.y), 0.0)
        # The false (0) will be unchanged, so we will have 3/5 correct
        self.assertEqual(utils.accuracy(self.y, self.y * 0.5), 0.6)
        
    def test_precision(self):
        self.assertEqual(utils.precision(self.y, self.y), 1.0)
        
        # True positives = 0, False positives = 5, so precision = 0
        self.assertEqual(utils.precision(self.y, 1-self.y), 0.0)
        
        # True positive = 0, False positive = 0, so precision = 0, should handle division by zero
        self.assertEqual(utils.precision(y_true=[1,1,1,1,1], y_pred=[0,0,0,0,0]), 0.0)
        
        # True positive = 2, False positive = 2, so precision = 0.5
        self.assertEqual(utils.precision(y_true=[1,1,0,0], y_pred=[1,1,1,1]), 0.5)
