...
class Tensor:
    ...
    def grad(self) -> Optional["Tensor"]:
        ...
        as a NumPy it will be wrapped in a `Tensor` before returning.
    ...
    def expand(self, *shape: Union[int, Sequence[int]]) -> "Tensor":
        ...
        This operation broadcasts the input tensor to a new shape. The forward pass creates a new mx
    ...
    def _accumulate_grad(
        self, grad: Union["Tensor", Array], idx: Optional[Any] = None
    ) -> None:
        """
        ...
        Args:
            grad (Union[Tensor, xp.ndarray]): The gradient to accumulate.
...
class IAdd(Function):
    ...
    def backward(self, grad: "Tensor") -> Tuple[Array, Array]:
        ...
        Returns:
            Tuple[xp.ndarray, xp.ndarray]: The gradients with respect to ``x`` and ``y``.
...
class Sqrt(Function):
    ...
    def forward(self, x: Array) -> Array:
        ...
        Returns:
            xp.ndarray: The element-wise square root of ``x``.
        """
    ...
    def backward(self, grad: "Tensor") -> Array:
        ...
        if self.axis is not None:
            num_elements = xp.prod(
                xp.array([self.tensors[0].shape[ax] for ax in self.axis])
            )
        else:
            num_elements = 1
...
class Gather(Function):
    ...
    def forward(self, x: Array, index: Array) -> Array:
        ...
        if index_values.size and (
            bool(xp.any(index_values < 0)) or bool(xp.any(index_values >= x.shape[0]))
        ):
    ...
    def backward(self, grad: "Tensor") -> Tuple[Array, None]:
        ...
        Returns:
            Tuple[xp.ndarray, None]: A tuple where the first element is the gradient with respect to
...
class View(Function):
    ...
    def forward(
        self, x: Array, new_shape: Union[Tuple[int, ...], List[int]] = (1,)
    ) -> Array:
        ...
        if x.size != xp.prod(xp.array(new_shape)):
            raise ValueError(
                f"Size of new view must match size of original tensor: {x.size} != {xp.prod(xp.array(new_shape))}"
            )
...
class Expand(Function):
    ...
    def forward(
        self, x: Array, shape: Union[Tuple[int, ...], List[int]] = (1,)
    ) -> Array:
...
class StridedWindows(Function):
    ...
    def forward(self, x: Array, kernel_size: int, stride: int) -> Array:
        ...
        if hasattr(x, "strides") and hasattr(x, "itemsize"):
            ...
            # Backends like MLX do not expose array strides publicly. Materialize a
            # contiguous copy so the view math below matches the actual layout.
            x = xp.array(x)
        ...
        return xp.as_strided_view(
            x,
            shape=(H_out, W_out, batch_size, channels, kernel_size, kernel_size),
            strides=(
                base_strides[2] * stride,  # steps in the height dimension
                base_strides[3] * stride,  # steps in the width dimension
                base_strides[0],  # batch dimension stride
                base_strides[1],  # channels dimension stride
                base_strides[2],  # inside window vertical steps
                base_strides[3],  # inside window horizontal steps
            ),
        )
